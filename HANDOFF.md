...
| 2 | Claude | ~~회색 박스 전혀 안 뜸 + **첨부만 있는 질문은 마커 자체가 안 생김**~~ **수정 완료 (2026-06-04, 실기기 확인 대기)** | DOM 샘플로 확정: 썸네일(`[data-testid="file-thumbnail"]`, 파일명은 내부 `h3`)이 버블 밖 턴 래퍼에 있음. (1) getQuestionText 컨테이너를 `[data-test-render-count]`로 확장, 본문 제외에 `.sr-only`(You said 중복)/썸네일/`[role="group"]`(타임스탬프) 추가 → 회색 박스 해결(실기기 확인). (2) 첨부 전용 질문: 라이브 검증으로 **턴 래퍼([data-test-render-count])와 user-message 둘 다 없이 썸네일 그리드(`div.grid.grid-cols-[...]`)만 렌더**됨을 확인 → 래퍼 밖 썸네일의 `closest('div[class*="grid-cols"]')` 그리드를 질문 요소로 수집 (form 내부=입력창 첨부 대기는 제외). 턴 래퍼 기반 수집과 `getPlainIdentity` 썸네일 h3 폴백(content.js)은 안전망으로 유지 |
